...
public class MapUtil {
     ...
     * @param value
     */
    public static void fillColValue(GridData<?> mData, String column, Object value){
        int size = mData.size();
        for(int i = 0; i < size; i++){
            if(mData.get(i) instanceof HashMap){
                ((HashMap)mData.get(i)).put(column, value);
            }else{
                try{
                    Object aObj = mData.get(i);
                    String methodName = "set" + column.substring(0, 1).toUpperCase()
                            + column.substring(1, column.length());
                    ClassUtils.invokeSimple(aObj, methodName, value);
                }catch(Exception e){
            ...
            for(int i = 0; i < mData.size(); i++){
                ...
                while(dataIterator.hasNext()){
                    ...
                    if(str == null){
                        ...
                        str = StringUtil.replace(str.toString(), ",", "`");
                    }
                    data += str + ",";
                }
    ...
    public static List<Map<String, Object>> toMap(List list){
        ...
        try{
            ...
            while(dataIterator.hasNext()){
                ...
                String strArry[] = StringUtil.split(str, ",", true);
                if(head == null){
                    head = strArry.clone();
                }else{
