...
public class SecurityUtil extends SeedBase {
    ...
    public byte[] CBCEncrypt( byte[] data ) {
        byte[] byteRetKey = { (byte)0x01, (byte)0x23, (byte)0x45, (byte)0x67, (byte)0x89, (byte)0xAB, (byte)0xCD, (byte)0xEF, (byte)0x12, (byte)0x34, (byte)0x56, (byte)0x78, (byte)0X9A, (byte)0XBC,
                (byte)0XDE, (byte)0XF0 };
    ...
    public static String base64Encode( String vo ) {
        return new String(org.apache.commons.codec.binary.Base64.encodeBase64(org.apache.commons.codec.binary.StringUtils.getBytesUtf8(vo)));
    }
     ...
     * @return
     */
    public static String makeUniqueKey( String args ) {
        return getMessageDigest("SHA1", args);
    }
     ...
     * @throws Exception
     */
    public static String pwdEncode( String input ) {
        String rstPwd = "";
        String encodingType = Constants.initPageEncodingType;
        if(encodingType.equals("hash")) {
            rstPwd = makeUniqueKey(input);
        } else if(encodingType.equals("encode")) {
            rstPwd = Encode(input);
        }
        return rstPwd;
    }
    ...
    // 외부 웹시스템 인증용 id 생성
    public static String encodeForExt(String userId) {

        String key = "dwtzkey2013|" + userId;
        key = SecurityUtil.base64Encode(key);
        //trace("key1:" + key);

        String keyCreateDt = DateUtil.getCurrentDateString("yyyyMMddHH");
    ...
    // 외부 웹시스템 인증용 id 복원
    public static String[] decodeForExt(String key) {
        String keyInfo[] = {"", "", ""};

        //trace('key1:' + key);
        key = SecurityUtil.base64Decode(key);
        //trace('key2:' + key);
        String keyCreateDt = key.substring(key.indexOf('|') + 1, key.length());
        ...
        key = SecurityUtil.base64Decode(key);
        //trace('key4:' + key);
        String sysCd = key.substring(0, key.indexOf('|'));
        String userId = key.substring(key.indexOf('|') + 1, key.length());

        keyInfo[0] = sysCd;
        keyInfo[1] = userId;
        keyInfo[2] = keyCreateDt;
