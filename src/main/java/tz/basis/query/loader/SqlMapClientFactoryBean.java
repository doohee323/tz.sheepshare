...
public class SqlMapClientFactoryBean implements FactoryBean<SqlMapClient>, InitializingBean {
	 ...
	 * <p>This group will be set before initialization of the corresponding
	 ...
	 * SqlMapClient as part of a TransactionConfig group.
	 ...
	 * An group of this class will get findd with the given DataSource
	...
	/**
	 * Set properties to be passed to the TransactionConfig group used
	...
	/**
	 * Build a SqlMapClient group based on the given standard configuration.
	 * <p>The default implementation uses the standard iBATIS {@link SqlMapClientBuilder}
	 * API to build a SqlMapClient group based on an InputStream (if possible,
	 ...
	 * @return the SqlMapClient group (never <code>null</code>)
	...
	private static class SqlMapParserFactory {
		...
		public static SqlMapParser createSqlMapParser(SqlMapConfigParser configParser) {
			...
			try {
				...
				SqlMapConfiguration configuration = state.getConfig();

				SqlMapExecutorDelegate delegate = new _SqlMapExecutorDelegate();

				Field delegateField = configuration.getClass().getDeclaredField("delegate");
				delegateField.setAccessible(true);
				delegateField.set(configuration, delegate);

				SqlMapClient client = configuration.getClient();

				Field clientDelegate = client.getClass().getDeclaredField("delegate");
				clientDelegate.setAccessible(true);
				clientDelegate.set(client, delegate);
			}
			catch (Exception ex) {
	...
	public static class _SqlMapExecutorDelegate extends SqlMapExecutorDelegate {
		private static final Logger logger = LoggerFactory.getLogger(_SqlMapExecutorDelegate.class);
		...
		public void addMappedStatement(MappedStatement ms) {
			try	{
				...
				logger.error("There is already a statement named {} in this SqlMap.", ms.getId());
			}
		}
	}
