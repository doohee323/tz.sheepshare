...
public interface UploadEventHandler {
	 ...
	 *            받은 파일에 대한 관리자.
	 */
	void postprocess(String folder, MultiPartInfo info,
			PersistenceManager persistenceManager);

	/**
	 * 멀티파트 업로드 요청 안의 개별 파일에 대한 Persistence 이름을 결정.
	 * <p>
	 * 다운로드 서블릿 구현시 PersistenceManager에 저장할 때의 folder/callName과 불러올 때의
	 * folder/callName은 동일해야 함.
	 * 
	 * @param folder
	 * @param fileInfo
	 * @return
	 ...
	 * <p>
	 * 멀티파트 파일 요청이므로 여러 개의 파일을 한 요청에서 업로드할 수 있으므로 해당 folder에 여러 개의 파일을 중복해서 올리는
	 * 등의 예외 사항을 처리해야 함.
	 * <p>
